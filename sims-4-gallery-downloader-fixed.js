// ==UserScript==
// @name        TS4 gallery downloader (fixed for truncated data)
// @description Download households, lots and rooms from The Sims 4 Gallery website. Includes fix for truncated .dat files.
// @author      anadius + minor fix
...
(function() {
  ...
  (async () => {
    ...
    const parseValue = (value, fieldType, isParentArray) => {
      let _;  // we don’t really use this underscore except for destructuring
    ...
    // EA likes to mix snake/camel case, so we have to handle both.
    ...
    const getTrayItem = async (uuid, guid, folder) => {
      ...
      if(parsedMessage.type === EXCHANGE_BLUEPRINT) {
        ...
        // the same logic for new IDs is used later in the data file
    ...
    const getDataItem = async (guid, folder, type, id) => {
      ...
      // For Households, fix up sim IDs, but guard against truncated data
      if(type === EXCHANGE_HOUSEHOLD) {
        const messageClass = root.lookupTypeOrEnum('EA.Sims4.Network.FamilyData');

        // read the .dat structure
        const totalBytes = response.byteLength;
        if (totalBytes < 8) {
          console.error("Truncated .dat file: fewer than 8 bytes, skipping re-encode.");
          // Return the raw data so we at least have something in the ZIP
          return response;
        }
        ...
        if (totalBytes < 8 + len) {
          // The .dat is smaller than we expected – avoid RangeError
          console.error(`Truncated .dat file: needed ${8 + len} bytes, but only got ${totalBytes}. Skipping re-encode.`);
          // Return the raw data to avoid crashing
          return response;
        }

        // If it’s large enough, decode + fix IDs
        ...
        try {
          ...
          console.error("Error re-encoding the .dat file:", err);
          return response; // fallback
        }
      }
      else {
        // For non-households, just return as-is
        return response;
      }
    };
    ...
    const loadImage = url => new Promise(resolve => {
      xhr({
        ...
        // If the image fails, just resolve null or handle it
        resolve(null);
      });
    });
    ...
    const getImages = async (guid, folder, type, additional) => {
      ...
      for(let i=0; i<=additional; ++i) {
        ...
        // If an image is missing, skip it
        if (!img) {
          console.warn("Missing or invalid image at URL:", url);
          continue;
        }
    ...
    console.log('TS4 gallery downloader running with truncated-data fix');
